...
df_obt_trat <- select(df_obt_trat, Procedimentos_hepC_CID_óbitos = tratamentoCID, Var1, Freqt)
...
ggplot(dr_proc_uf, aes(x=reorder(UF, -group_sum), y=Freq, fill=tratamentoCID)) +
  ...
  geom_text(aes(label=Freq), position = position_stack(vjust = 0.8), size=2.5)+
  theme_minimal()  + labs(x="UF", y = "Frequência") + scale_color_brewer(palette = "Paired") + scale_y_log10()
